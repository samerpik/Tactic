...
def line_mask(frame):
    ...
    for ang in range(0, 180, 15):
        k = np.zeros((31, 31), np.uint8)
        c = 15
        dx, dy = np.cos(np.radians(ang)), np.sin(np.radians(ang))
        cv2.line(k, (int(c-14*dx), int(c-14*dy)), (int(c+14*dx), int(c+14*dy)), 1, 1)
...
def refine_frame(frame, H_img2pitch, stage_iters=14):
    """Return refined image->pitch homography.

    Three trimmed LM stages with shrinking capture radius: samples that stay
    saturated at the cap (their model line isn't visible in this view) are
    dropped between stages so they stop dragging the fit."""
    mask = line_mask(frame)
    dt = dt_of(mask)
    all_pts = model_samples()
    pts = all_pts
    ...
    for cap in (30.0, 16.0, 8.0):
        ...
        for it in range(stage_iters):
            ...
            if vis_new.sum() > 50 and (r_new[vis_new]**2).sum() / vis_new.sum() < cost:
                ...
                lam = min(lam * 4, 1e3)
        # trim: keep only samples whose line was actually found at this scale
        r, vis = residuals(unpack(p), pts, dt, cap)
        keep = vis & (r < cap * 0.9)
        if keep.sum() >= 80:
            pts = pts[keep]
    r, vis = residuals(unpack(p), pts, dt, 8.0)
    inl = vis & (r < 7.2)
    mean_px = float(r[inl].mean()) if inl.sum() else -1
